...
final_net.localMorans <- 
  cbind(
    as.data.frame(localmoran(final_net$countArrest, final_net.weights)),
    as.data.frame(final_net)) %>% 
    st_sf() %>%
      dplyr::select(Robbery_Count = countArrest, 
                    Local_Morans_I = Ii, 
                    P_Value = `Pr(z > 0)`) %>%
...
ggplot(correlation.long, aes(Value, countArrest)) +
  geom_point(size = 0.1) +
