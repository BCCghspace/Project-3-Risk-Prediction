...
for(i in vars){
  mapList[[i]] <- 
    ggplot() +
      ...
      labs(title=i) +
      mapTheme()}   #AD: does this work for you?
...
##exploring the spatial process of arrests##
...
final_net <-
  ...
  mutate(arrests.isSig = 
           ifelse(localmoran(final_net$countArrest, 
...
correlation.long <-
  st_drop_geometry(final_net) %>%
    ...
    gather(Variable, Value, -countArrest)

correlation.cor <-
  correlation.long %>%
    group_by(Variable) %>%
    summarize(correlation = cor(Value, countArrest, use = "complete.obs"))
...
final_net.localMorans <- 
  cbind(
    ...
    st_sf() %>%
      dplyr::select(Arrest_Count = countArrest, 
                    Local_Morans_I = Ii, 
                    P_Value = `Pr(z > 0)`) %>%
